#!/bin/bash
#Don't use this script unless you're pretty bloody sure what you're doing.
...
newAddress() {
	...
	dc -e "$ec_dc lG I16i${exponent^^}ri lMx 16olm~ n[ ]nn" |
	{
	    ...
	    echo "---DON'T USE THIS UNLESS YOU KNOW WHAT YOU'RE DOING---"
      echo "----------NOT MY FAULT IF YOU SCREW IT UP-------------"
            echo "input exponent:          0x$exponent"
	    echo "Secp256k1 coordinates:"
	    echo "    X:                         $X"
	    echo "    Y:                         $Y"
      echo "Dont bloody lose this:"
      echo "    Private Key:               $compressed_wif"
      echo "    Public Address:            $compressed_addr"
	}
    elif test -z "$1"
